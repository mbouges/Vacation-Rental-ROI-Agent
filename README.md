...
- Manual-entry fallback prompts when extraction is blocked, low-confidence, or unusable
...
3. If extraction is blocked or low-confidence, use `manual_entry_prompt` to collect confirmed facts from the user.
4. Collect any missing assumptions you want to confirm with the user.
5. Call `analyze_property` with the property details and final assumptions.
6. Use the returned `analysis_id` with `answer_followup` for what-if questions.
...
- `manual_entry_prompt` when the extractor needs the user to provide or confirm property details directly
